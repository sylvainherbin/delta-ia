...
3. **Commentaires à revoir (D64).** `.venv/bin/python scripts/catalogue.py a-commenter --perimetre openai --lot perimees` donne au plus 30 entrées `utiliser` ou `tester` : d'abord celles dont une section citée de CONTEXTE.md a changé ou disparu, puis celles commentées avant D64 (`contexte_sections: null`). Réévalue-les en citant leurs sections, applique, valide, puis commite à part : `delta-kb(openai): réévaluation contexte — <n> entrées`. Les autres entrées antérieures à D64 ne sont pas reprises en bloc. Ces réévaluations s'ajoutent aux deux lots.
4. **Nouveau projet dans CONTEXTE §2 (D64).** Si `catalogue.py lots --perimetre openai` affiche une ligne `nouveau-projet:<clé>`, relis une fois, en gabarit court, les entrées « ignorer » des fonctionnalités et commandes listées par `a-commenter --lot nouveau-projet:<clé>` : le verdict monte si le nouveau projet change quelque chose de concret, et `contexte_sections` cite la clé du projet dans tous les cas.
