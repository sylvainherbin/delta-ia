...
1. **Adoptions déclarées (D67).** `.venv/bin/python scripts/catalogue.py adoptions --perimetre openai` lit la section « Adoptions » de `PROGRESSION.md` et passe `statut_usage` à `utilise` pour les id qu'elle cite, avec la mention « adoption déclarée, PROGRESSION.md » dans `historique` ; il signale un id absent de la base. S'il change une entrée, `valider.py --kb`, puis commit à part : `delta-kb(openai): adoptions (PROGRESSION.md)`. Rien d'autre dans `PROGRESSION.md` n'agit sur la base.
2. **Recalibrage ponctuel (D57).** Si `git log --oneline --grep 'recalibrage commandes'` ne renvoie rien, commence par réévaluer selon D57 **toutes** les entrées dont le verdict est `utiliser` (il y en avait 26 le 23/09) : `.venv/bin/python -c "import json,glob; print(json.dumps([e for f in glob.glob('docs/data/kb/openai/*.json') for e in json.load(open(f))['entrees'] if e.get('commentee') and e['recommandation']['verdict']=='utiliser'], ensure_ascii=False, indent=1))"`. Applique les corrections avec `scripts/catalogue.py appliquer`, valide avec `valider.py --kb`, puis commite à part : `delta-kb(openai): recalibrage commandes`, et pousse. Cette étape n'a lieu qu'une fois.
3. **Recalibrage ponctuel de `tester` (D57 étendu).** Si `git log --oneline --grep 'recalibrage tester'` ne renvoie rien, réévalue avec cette règle, avant tout nouveau lot et en particulier avant `parametres:4`, toutes les entrées `parametres` dont le verdict est `tester` (111 le 24/09) : `.venv/bin/python -c "import json; print(json.dumps([e for e in json.load(open('docs/data/kb/openai/parametres.json'))['entrees'] if e.get('commentee') and e['recommandation']['verdict']=='tester'], ensure_ascii=False, indent=1))"`. Applique avec `scripts/catalogue.py appliquer`, valide avec `valider.py --kb`, puis commite à part : `delta-kb(openai): recalibrage tester`, et pousse. Cette étape n'a lieu qu'une fois.
4. **Lot perimees (D64-bis).** Si `catalogue.py lots --perimetre openai` affiche une ligne `perimees`, traite d'abord ce lot, 30 entrées au plus par lancement : `.venv/bin/python scripts/catalogue.py a-commenter --perimetre openai --lot perimees`. Chaque entrée porte un `motif` : `section:<ctx-id>` (section citée modifiée ou dépréciée : relis-la), `adoption` (verdict `ignorer` alors que Sylvain déclare utiliser l'entrée dans PROGRESSION.md, D67 : revois le verdict à la lumière de cet usage), `legacy` (commentaire antérieur à D64, `utiliser` ou `tester`), `age` (filet par âge : dernier commentaire de plus de 90 à 179 jours, tout verdict). Recommente-la entièrement selon les règles ci-dessous, avec des `contexte_sections` à jour ; `catalogue.py appliquer` inscrit chaque réévaluation dans `docs/data/kb/openai/reevaluations.jsonl`.
5. **Nouveau projet dans CONTEXTE (D64-bis).** Si `catalogue.py lots --perimetre openai` affiche une ligne `nouveau-projet:<ctx-id>`, relis une fois, en gabarit court, les entrées « ignorer » des fonctionnalités et commandes listées par `a-commenter --lot nouveau-projet:<ctx-id>` : le verdict monte si le nouveau projet change quelque chose de concret, et `contexte_sections` cite le ctx-id du projet dans tous les cas.
...
- **Sections de CONTEXTE citées (D64-bis)** : chaque commentaire porte `contexte_sections`, `{ctx-id: pourquoi}`, avec pour chaque section une ligne de 160 caractères au plus qui dit ce que le commentaire en tire. Cite le ctx-id **le plus précis** (`config.codex.profils` plutôt que `config.codex`, `projet.trading-sim` plutôt que `projet.vue-ensemble`) ; une section au corps vide (titre seul, marquée « corps vide » par `scripts/contexte.py`) ne se cite jamais : cite la sous-section ; `.venv/bin/python scripts/contexte.py` les liste. Une entrée qui ne dépend d'aucune section a `{}` : le filet par âge la reprendra. `catalogue.py appliquer` refuse un ctx-id inconnu ou déprécié et un pourquoi vide, et calcule lui-même le sha1 de chaque section. Échange assumé : une section omise ne périme rien, d'où le pourquoi par section, le journal et le filet par âge.
- **Empreinte du contexte** : `scripts/catalogue.py appliquer` inscrit aussi l'empreinte du fichier entier (`contexte_empreinte`, historique de D60) ; tu ne la fournis pas.
...
- **Écriture** : un fichier JSON `{id: {description, statut_usage, recommandation: {verdict, pourquoi}, contexte_sections: {ctx-id: pourquoi}, exemple, disponibilite}}`, appliqué par `.venv/bin/python scripts/catalogue.py appliquer --perimetre P --fichier <fichier>`. Le script refuse tout autre champ et rien n'est écrit en cas d'erreur.

## 4. Compte rendu (10 lignes au plus)

Réévaluations faites (recalibrage, entrées périmées par motif) et **taux de verdicts changés** (`.venv/bin/python scripts/catalogue.py reevaluations --perimetre openai`, B3) ; lots traités ; entrées commentées par verdict (`utiliser`, `tester`, `ignorer`) et par `statut_usage` ; les `utiliser` en une ligne chacun ; les entrées dont `usage` paraît faux ou incomplet ; les lots restants et l'avancement `n/N` (`catalogue.py inventaire --perimetre openai`).
