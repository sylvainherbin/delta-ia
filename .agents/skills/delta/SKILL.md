...
- **Sections citées (D64)** : chaque élément porte `contexte_sections`, les sections de CONTEXTE.md sur lesquelles reposent `pour_toi`, `impact` et `action`, au format `{clé: sha1}` donné par `.venv/bin/python scripts/contexte.py --sections 2.2,4` (`{}` si l'élément ne dépend pas de CONTEXTE).
