...
- **Articles de la newsroom Anthropic (A1, 26/09)** : pour une source à option `lire_articles` (`anthropic-newsroom`), chaque nouvel article est lu (un GET) et son texte principal (plus long `<article>` de la page, 20 000 caractères au plus) remplace le résumé de la liste dans `contenu`. Un gabarit changé, un texte vide ou trop court, ou un article illisible remonte en `sources_en_echec` (partiel), jamais en contenu vide.
- **Pages suivies par section (O2, 26/09)** : format `sections_suivies` avec `suivre_revisions` ; chaque section déclarée dans `options.sections` est un élément non daté `<source>-<id>` qui revient en révision quand son texte change ; au premier passage, l'état actuel de chaque section arrive une fois ; une section introuvable ou vide remonte en `sources_en_echec`. Source `openai-tarifs` (https://learn.chatgpt.com/docs/pricing.md, périmètre `openai`, `a_valider`) : plans et crédits, parrainage, limites.
...
| `statut_usage` | `utilise` \| `non_utilise` \| `inconnu`, d'après CONTEXTE.md ; `inconnu` avant commentaire ; `utilise` aussi pour un id cité dans la section « Adoptions » de PROGRESSION.md (`catalogue.py adoptions`, historique « adoption déclarée, PROGRESSION.md », D67). Un essai unique ne modifie jamais `statut_usage` : « testé » s'exprime dans `recommandation.pourquoi` et dans la provenance, et le schéma `utilise` \| `non_utilise` \| `inconnu` est inchangé (D69) |
...
Notifications, retour utile/inutile, hébergement privé, automatisation par cron. Exception en essai (D68) : le passage `/delta` de Claude Code (périmètres `claude` et `actu`) peut être lancé par une tâche planifiée locale de Claude Desktop ; `$delta`, `/delta-kb` et `$delta-kb` restent manuels. Pour le connecteur MCP (D61) : toute écriture, toute authentification et tout déclenchement de passage à distance restent hors périmètre ; un outil qui en aurait besoin exige une nouvelle décision.
...
| D67 | Formation : `PROGRESSION.md`, tenu par la seule session professeur, en lecture seule pour les agents ; la session professeur le commite elle-même, ce fichier seul, avec un `git add` explicite (amendement du 24/09 ; `CONTEXTE.md` reste sous D32) ; `/delta` et `$delta` le lisent pour doser la profondeur de `pour_toi` et `action` (acquis : pas de réexplication ; à travailler : plus détaillé), sans effet sur l'impact, le choix ni le tri des éléments, et sans empreinte ; `/delta-kb` et `$delta-kb` passent en `utilise` les id de sa section « Adoptions » (`catalogue.py adoptions`), consignés dans `historique` ; rien d'autre dans ce fichier n'agit sur la base | §4, §7.4, REGLES §1, skills |
| D68 | Essai d'une semaine du passage `/delta` automatique (`claude` et `actu` seulement) : tâche planifiée locale de Claude Desktop dont la consigne lit `.claude/skills/delta/SKILL.md` et suit sa section « Mode automatique (D68) », sans invoquer la skill (option A : `disable-model-invocation: true` empêche une tâche planifiée de lancer `/delta`) ; `scripts/garde.py` en premier (lecture seule ; arrêt si `.git/index.lock`, quota Claude session 5 h ≥ 80 % ou semaine ≥ 85 %, passage du jour déjà fait ; relevé `usage.json` de plus de 15 min signalé sans arrêt) ; aucune question, toute erreur arrête le passage sans push (sauf `fetch.py --kb` en code 3, échec partiel : avertissement, pages en échec listées dans le rapport, entrées des pages réussies commitées ; le code 5 et un `valider.py --kb` en échec restent bloquants), un seul push à la fin, rapport `…-delta-ia-delta-auto.md` même en cas d'arrêt ; liste `allow` précise dans `.claude/settings.json` (scripts du passage, `git pull --rebase`, deux `git add` et deux `git commit` exacts, `git push`, écritures limitées aux dossiers du passage), sans `bypassPermissions` ni joker large ; mode de permission de la tâche : Manual | §10, skill, `.claude/settings.json` |
| D69 | Projet Routage (chefs et sous-agents) : une règle empirique qui en est issue n'est promouvable vers CONTEXTE.md, par défaut, qu'après au moins 3 observations concordantes couvrant au moins 2 types de tâches, avec une comparaison appropriée et un coût complet (chef, exécutant, vérification et reprise) ; exception possible seulement lorsqu'une preuve déterministe ou une évidence plus forte rend les répétitions artificielles, et explicitement justifiée. Les résultats bruts restent dans le projet Routage et n'entrent dans Delta que par CONTEXTE.md (D32). Un essai unique ne modifie jamais `statut_usage` : « testé » s'exprime dans le `pourquoi` et la provenance, le schéma `utilise` \| `non_utilise` \| `inconnu` est inchangé | §7.4, CONTEXTE.md (D32) |
