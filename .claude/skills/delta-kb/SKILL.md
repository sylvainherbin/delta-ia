...
1. **Adoptions déclarées (D67).** `.venv/bin/python scripts/catalogue.py adoptions --perimetre claude` lit la section « Adoptions » de `PROGRESSION.md` et passe `statut_usage` à `utilise` pour les id qu'elle cite, avec la mention « adoption déclarée, PROGRESSION.md » dans `historique` ; il signale un id absent de la base. S'il change une entrée, `valider.py --kb`, puis commit à part : `delta-kb(claude): adoptions (PROGRESSION.md)`. Rien d'autre dans `PROGRESSION.md` n'agit sur la base.
2. **Lot perimees (D64-bis).** Si `catalogue.py lots --perimetre claude` affiche une ligne `perimees`, traite d'abord ce lot, 30 entrées au plus par lancement : `.venv/bin/python scripts/catalogue.py a-commenter --perimetre claude --lot perimees`. Chaque entrée porte un `motif` : `section:<ctx-id>` (section citée modifiée ou dépréciée : relis-la), `adoption` (verdict `ignorer` alors que Sylvain déclare utiliser l'entrée dans PROGRESSION.md, D67 : revois le verdict à la lumière de cet usage), `legacy` (commentaire antérieur à D64, `utiliser` ou `tester`), `age` (filet par âge : dernier commentaire de plus de 90 à 179 jours, tout verdict). Recommente-la entièrement selon les règles ci-dessous, avec des `contexte_sections` à jour ; `catalogue.py appliquer` inscrit chaque réévaluation dans `docs/data/kb/claude/reevaluations.jsonl`.
3. **Nouveau projet dans CONTEXTE (D64-bis).** Si `catalogue.py lots --perimetre claude` affiche une ligne `nouveau-projet:<ctx-id>`, relis une fois, en gabarit court, les entrées « ignorer » des fonctionnalités et commandes listées par `a-commenter --lot nouveau-projet:<ctx-id>` : le verdict monte si le nouveau projet change quelque chose de concret, et `contexte_sections` cite le ctx-id du projet dans tous les cas.
